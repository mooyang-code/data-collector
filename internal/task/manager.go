...
func (m *manager) Start(ctx context.Context) error {
	...
	if err := m.loadTasks(); err != nil {
		m.logger.Warn("failed to load tasks", "error", err)
	}
	...
	for _, task := range m.tasks {
		if task.Status == model.TaskStatusRunning {
			if err := m.scheduleTask(task); err != nil {
				m.logger.Error("failed to reschedule task", "task_id", task.ID, "error", err)
			}
		}
	}
	...
	m.started = true
	m.logger.Info("task manager started", "task_count", len(m.tasks))
	return nil
}
...
func (m *manager) Stop(ctx context.Context) error {
	...
	if err := m.saveTasks(); err != nil {
		m.logger.Error("failed to save tasks on stop", "error", err)
	}
...
func (m *manager) CreateTask(ctx context.Context, task *model.Task) error {
	...
	// 保存到文件
	if err := m.saveTasks(); err != nil {
		m.logger.Error("failed to save tasks after create", "error", err)
	}
...
func (m *manager) UpdateTask(ctx context.Context, task *model.Task) error {
	...
	m.logger.Info("updating task", "task_id", task.ID)
	...
	if task.Status == model.TaskStatusRunning {
		if err := m.scheduleTask(task); err != nil {
			m.logger.Error("failed to reschedule updated task", "task_id", task.ID, "error", err)
	...
	// 保存到文件
	if err := m.saveTasks(); err != nil {
		m.logger.Error("failed to save tasks after update", "error", err)
	}
...
func (m *manager) DeleteTask(ctx context.Context, taskID string) error {
	...
	m.logger.Info("deleting task", "task_id", taskID)
	...
	// 保存到文件
	if err := m.saveTasks(); err != nil {
		m.logger.Error("failed to save tasks after delete", "error", err)
	}
...
func (m *manager) StartTask(ctx context.Context, taskID string) error {
	...
	m.logger.Info("starting task", "task_id", taskID)
	...
	if err := m.scheduleTask(task); err != nil {
		task.Status = model.TaskStatusError
		m.logger.Error("failed to schedule task", "task_id", taskID, "error", err)
		return fmt.Errorf("failed to schedule task: %w", err)
	}
	
	// 保存状态
	if err := m.saveTasks(); err != nil {
		m.logger.Error("failed to save tasks after start", "error", err)
	}
...
func (m *manager) StopTask(ctx context.Context, taskID string) error {
	...
	m.logger.Info("stopping task", "task_id", taskID)
	...
	// 保存状态
	if err := m.saveTasks(); err != nil {
		m.logger.Error("failed to save tasks after stop", "error", err)
	}
...
func (m *manager) loadTasks() error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			m.logger.Info("task file not found, starting with empty task list", "path", m.config.StorePath)
			return nil
		}
	...
	m.logger.Info("tasks loaded from file", "path", m.config.StorePath, "count", len(tasks))
	return nil
}
...
func (m *manager) saveTasks() error {
	...
	m.logger.Debug("tasks saved to file", "path", m.config.StorePath, "count", len(tasks))
	return nil
}
...
func (m *manager) unscheduleTask(taskID string) {
	if err := m.scheduler.RemoveTask(taskID); err != nil {
		// 任务不存在时不记录错误，这是正常情况
		m.logger.Debug("task not found for unscheduling", "task_id", taskID)
	} else {
		m.logger.Debug("task unscheduled", "task_id", taskID)
	}
}
...
func (m *manager) executeTask(taskID string) {
	...
	default:
		m.logger.Warn("task execution skipped due to concurrency limit", "task_id", taskID)
		return
	}
	...
	if !exists {
		m.mu.Unlock()
		m.logger.Error("task not found during execution", "task_id", taskID)
		return
	}
	...
	if _, running := m.runningTasks[taskID]; running {
		m.mu.Unlock()
		m.logger.Warn("task already running, skipping execution", "task_id", taskID)
		return
	}
...
func (m *manager) doExecuteTask(execution *taskExecution) {
	...
	defer func() {
		...
		m.logger.Info("task execution completed", 
			"task_id", taskID, 
	...
	if err != nil {
		m.logger.Error("failed to get collector", "task_id", taskID, "exchange", execution.task.Exchange, "error", err)
	...
	if err != nil {
		m.logger.Error("task execution failed", "task_id", taskID, "error", err)
		m.updateTaskStats(taskID, time.Since(startTime), err)
		return
	}
...
func (m *manager) stopTaskExecution(taskID string) {
	if execution, exists := m.runningTasks[taskID]; exists {
		execution.cancel()
		delete(m.runningTasks, taskID)
		m.logger.Info("task execution stopped", "task_id", taskID)
	}
}
...
func (m *manager) updateTaskStats(taskID string, duration time.Duration, execErr error) {
	...
	go func() {
		if err := m.saveTasks(); err != nil {
			m.logger.Error("failed to save tasks after stats update", "error", err)
		}
	}()
}
...
func (m *manager) publishTaskEvent(eventType string, data interface{}) {
	if m.eventBus != nil {
		...
		if err := m.eventBus.PublishWithMetadata(eventType, data, metadata); err != nil {
			m.logger.Error("发布任务事件失败", "error", err, "type", eventType)
		} else {
			m.logger.Debug("已发布任务事件", "type", eventType)
		}
	}
}
...
func (m *manager) subscribeEvents() {
	if m.eventBus != nil {
		...
		if err := m.eventBus.Subscribe("config.changed", handler); err != nil {
			m.logger.Error("订阅配置变更事件失败", "error", err)
		}
	}
}
...
func (m *manager) handleConfigChange(data interface{}) {
	...
	if configType == "task_configs" {
		// 任务配置变更，重新加载任务
		m.logger.Info("handling task config change")
		// TODO: 实现配置变更处理逻辑
	}
}