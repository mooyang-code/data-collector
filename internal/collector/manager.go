...
func NewManager(log logger.Logger, metricsCollector metrics.Collector) Manager {
	return &manager{
		registry:   GetRegistry(),
		collectors: make(map[string]Collector),
		logger:     log.With("component", "collector_manager"),
...
func (m *manager) Create(ctx context.Context, name string, config json.RawMessage) (Collector, error) {
	...
	m.logger.Info("collector created", "name", name, "type", collector.Type())
...
func (m *manager) GetCollector(exchange string) (Collector, error) {
	...
	m.logger.Info("auto-created collector for exchange", "exchange", exchange, "type", collector.Type())
	return collector, nil
}
...
func (m *manager) Start(ctx context.Context, name string) error {
	...
	m.logger.Info("collector started", "name", name)
	return nil
}
...
func (m *manager) Stop(ctx context.Context, name string) error {
	...
	m.logger.Info("collector stopped", "name", name)
	return nil
}
...
func (m *manager) Remove(ctx context.Context, name string) error {
	...
	// 先停止采集器
	if err := collector.Stop(ctx); err != nil {
		m.logger.Warn("failed to stop collector during removal", "name", name, "error", err)
	}
	
	// 从管理器中移除
	delete(m.collectors, name)
	
	m.logger.Info("collector removed", "name", name)
	return nil
}
