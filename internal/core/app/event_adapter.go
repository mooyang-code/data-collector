...
func (a *EventBusAdapter) Publish(event Event) error {
	...
	return a.eventBus.Publish(&eventAdapter{appEvent: event})
}

// Subscribe 实现 app.EventBus 接口
func (a *EventBusAdapter) Subscribe(pattern string, handler func(Event) error) error {
	// 将处理器适配
	return a.eventBus.Subscribe(pattern, func(e event.Event) error {
...
func (e *eventAdapter) Context() context.Context { return context.Background() } // 默认返回Background