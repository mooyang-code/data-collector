...
// BaseCollectorOptimized 使用cron调度器的优化版本
type BaseCollectorOptimized struct {
	id       string
	typ      string
	dataType string
...
// NewBaseCollectorOptimized 创建优化的基础采集器
func NewBaseCollectorOptimized(id, typ, dataType string) *BaseCollectorOptimized {
	return &BaseCollectorOptimized{
		id:        id,
		typ:       typ,
		dataType:  dataType,
...
func (c *BaseCollectorOptimized) AddTimer(name string, interval time.Duration, handler TimerHandler) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *BaseCollectorOptimized) AddCronTimer(name string, cronExpr string, handler TimerHandler) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// Start 启动采集器
func (c *BaseCollectorOptimized) Start(ctx context.Context) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// Stop 停止采集器
func (c *BaseCollectorOptimized) Stop(ctx context.Context) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// GetStatus 获取状态（包含调度器信息）
func (c *BaseCollectorOptimized) GetStatus() CollectorStatus {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *BaseCollectorOptimized) wrapHandler(name string, handler TimerHandler) func(context.Context) error {
...
// 其他方法保持不变...
func (c *BaseCollectorOptimized) ID() string {
	return c.id
}

func (c *BaseCollectorOptimized) Type() string {
	return c.typ
}

func (c *BaseCollectorOptimized) DataType() string {
	return c.dataType
}

func (c *BaseCollectorOptimized) Initialize(ctx context.Context) error {
	return nil
}

func (c *BaseCollectorOptimized) RemoveTimer(name string) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *BaseCollectorOptimized) GetTimers() map[string]*Timer {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *BaseCollectorOptimized) IsRunning() bool {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *BaseCollectorOptimized) GetMetrics() CollectorMetrics {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *BaseCollectorOptimized) SetEventBus(eventBus EventBus) {
	c.eventBus = eventBus
}

func (c *BaseCollectorOptimized) PublishEvent(event interface{}) error {
	if c.eventBus == nil {
